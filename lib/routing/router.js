	...
	for(var key in routes) {
		...
		if(regexInfo) {
			...
			for(var req in routes[key]['requirements']) {	
				if(!(typeof optionals[req] !== 'undefined' && routes[key]['requirements'][req].test(optionals[req]))) { return false; }
			}
			...
			return { 'action': action, 'controller': controller, 'controllerName': key, 'optionals': optionals };
		}
	}
