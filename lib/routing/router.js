...
var NotFoundError = require('./../errors/notfound')
	, RouteError = require('./../errors/route');
	...
	for(var name in rawRoutes) {
		...
		for(var key in rawRoutes[name]) {
			if(key === 'route' && rawRoutes[name][key]) {		
				...
				if(controllerParts.length !== 2) throw new RouteError('A Route doesn\'t have two parts. The controller field should look like this controller:action');
...
function loadRoutes(file, callback) {
	fs.exists(file, function(exists) {
		if(exists) {
			...
			callback(new RouteError('Routes file doesn\'t exists. Check if the file exists or change the path in the settings file.'));
		}
	});
}
...
function findController(url, requestHandler, callback) {
	...
	for(var key in routes) {
		...
		if(regexInfo) {
			...
			controllerLoader.load(routes[key]['controller'], function(err, Controller) {
				...
				if(err !== null) {
					return(callback(new NotFoundError(err.msg)));
				}
				...
				for(var req in routes[key]['requirements']) {	
					if(!(typeof optionals[req] !== 'undefined' && routes[key]['requirements'][req].test(optionals[req]))) { 
						return(callback(new NotFoundError('A route was found, but the route didn\'t follow the requirements'))); 
					}
				}
				
				// Check if the action function exists in the controller (sanity check)
				if(typeof controller[action] !== 'function') {
					return(callback(new NotFoundError('The action exists in the controller, but it is not a function.')));
				}
	...
	if(!routeFound) {
		callback(new NotFoundError('No matching route was found.'));
	}
}
